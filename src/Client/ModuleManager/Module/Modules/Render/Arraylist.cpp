        ...
        return fontWidthA > fontWidthB; });
			...
			for (auto m : list)
			{
				...
				if (isEnabled)
				{
					...
					Color drawColor = rainbowEnabled->GetValue() ? HSVtoRGB(hue, 100, 100) : Color(255,255,255,255);

					G::Draw.Rect(startX - 4 - getFontWidth + m->animate, startY, getFontWidth, getFontHeight, Color(0, 0, 0, 170));
					G::Draw.Rect(startX - 4 + m->animate, startY, 10, getFontHeight, drawColor);
					G::Draw.String(EFonts::Greycliff_CF, startX - 4 + m->animate, startY, drawColor, TXT_LEFT, str.c_str());
